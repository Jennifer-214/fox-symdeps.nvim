...
function Hud:_asm()
  ...
  -- (ctx.container); on a CALL site the cursor is outside the namespace block,
  -- so fall back to the clangd hover ("// In namespace tt").
  local function with_container(container)
    if self.ctx.is_template then
      ...
      vim.ui.input({ prompt = ("asm: instantiate %s (e.g. <64>): "):format(self.ctx.symbol) }, function(args)
        if not args or vim.trim(args) == "" then return end
        args = vim.trim(args)
        if not args:match("^<") then args = "<" .. args end
        if not args:match(">$") then args = args .. ">" end
        self:_asm_run(container, args)
      end)
    else
      self:_asm_run(container, "")
    end
  end
  local c = self.ctx.container or ""
  if c ~= "" then
    with_container(c)
  else
    require("fox-symdeps.clangd").namespace_of(self.ctx, with_container)
  end
end
...
function Hud:_asm_run(container, targs)
  local flags = require("fox-symdeps.asmflags")
  local a, b = flags.pair()
  -- Qualify with the enclosing namespace so symbols inside `namespace tt { … }`
  -- compile; global-scope symbols get an empty container → unqualified name
  -- (identical to before). container comes from _asm (treesitter or the clangd
