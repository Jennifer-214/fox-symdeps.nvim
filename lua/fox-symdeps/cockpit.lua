...
local function maybe_open()
  ...
  if not panel.is_open() then
    panel.toggle((require("fox-symdeps").config or {}).palette)
  end
end
...
function M.toggle(from_setup)
  ...
  if M.enabled then
    ...
    if require("fox-symdeps.panel").is_open() then require("fox-symdeps.panel").close() end
  end
  if not from_setup then
    vim.notify("fox-symdeps · cockpit " ..
      (M.enabled and "ON (panel auto-docks on C++ buffers)" or "off"), vim.log.levels.INFO)
  end
end
